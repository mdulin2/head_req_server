...
def request_sender(url):
    ...
    trace_result = subprocess.run(["curl", "--insecure", "-v", "-X", "TRACE", url],stdout=subprocess.PIPE)
    print(trace_result)

    # Typically, this method is not even used.
    connect_result = subprocess.run(["curl", "--insecure", "-v", "-X", "CONNECT", url],stdout=subprocess.PIPE)
    print(connect_result)

    # This should give some sort of error.
    invalid_result = subprocess.run(["curl", "--insecure", "-v", "-X", "??", url],stdout=subprocess.PIPE)
